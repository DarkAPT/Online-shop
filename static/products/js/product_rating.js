...
const rating = parseFloat((ratingElement.getAttribute('data-rating')).replace(',', '.')); // Преобразуем строку в число

// Получаем контейнер для звездочек
...
function createStar(isFilled, ishalf, isEmpty, isFull) {
...
for (let i = 1; i <= totalStars; i++) {
    if (i <= Math.floor(rating)) { 
        starContainer.appendChild(createStar(true,false,false,true)); // Полная звезда
    } else if (i === Math.ceil(rating) && rating % 1 !== 0) {
        // Полузвезда
        const halfStar = createStar(true,true,false,false);
        starContainer.appendChild(halfStar);
    } else {
        starContainer.appendChild(createStar(true,false,true,false)); // Пустая звезда
    }
}